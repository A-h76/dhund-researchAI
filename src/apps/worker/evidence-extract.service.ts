...
export class EvidenceExtractService {
  ...
  async execute(
    ...
    const version = await this.extract.findVersion(payload.documentVersionId);
    if (version === null || version.projectId !== payload.projectId || version.deletedAt !== null) {
      this.metrics.recordExtractionFailure();
    ...
    const blocks = await this.extract.listBlocks(payload.documentVersionId);
  ...
  private async prepareCandidate(
    ...
    const block = await this.extract.findBlock(candidate.locator.blockId);
    if (locatorPointsAtBlock(candidate.locator, block) !== 'ok' || block === null) {
      return null;
    }
    ...
    if (quote.length === 0 || !quotedTextMatches(quote, block.text)) {
      return null;
    }
